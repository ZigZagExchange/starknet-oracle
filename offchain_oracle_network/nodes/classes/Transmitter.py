...
class Transmitter():
    ...
    async def send_transaction(self, account, to, selector_name, calldata, nonce=None):
        rrc, robs,  obs, r_sigs, s_sigs, signer_idxs = calldata
        if nonce is None:
        ...
        transmit = account.functions["transmit"].prepare(
            to, selector, rrc, robs,  obs, r_sigs, s_sigs, signer_idxs, nonce)
        return await transmit.invoke(signature=[sig_r, sig_s], max_fee=0)
...
def transmitter_hash_message(sender, to, selector, calldata, nonce):
    ...
    calldata_hash = compute_hash_on_elements(
        [rrc, robs] + obs + r_sigs + s_sigs + [signer_idxs])
